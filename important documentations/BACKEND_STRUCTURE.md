...
| `web_param_candidates(gobuster_results, dirb_results, max_candidates=6, probe_params=XSS_PROBE_PARAMS)` | gobuster/dirb brute-force *paths*, never query-string parameters, so nuclei's DAST XSS templates (and sqlmap) have nothing to mutate against a bare discovered path. This builds fuzzable `?param=` URLs from discovered script endpoints (`.php/.asp/.aspx/.jsp/.cgi`) × `probe_params`, capped at `max_candidates`. **Both** the XSS pass (default params, cap 6) **and** the sqlmap pass (`probe_params=("id",)`, cap 5, via `_detect_injectable_candidates()`) share this one ranker, so neither can regress the ordering fix independently. **Candidate ordering is load-bearing**: `_endpoint_rank()` sorts likely-parameterised endpoints (`search`, `view`, `id`, `info`, `sqli`, `xss`, ...) ahead of site roots (`index`, `home`) — without it six probes against `/index.php` consumed the whole budget and `/xss.php` / `/info.php` were never reached. Nothing is ever *excluded* by name, only ordered. |
...
|---|---|---|
| `cve_lookup.py` | `lookup_cves(product, version=None, target="nvd", limit=10)` and **`lookup_cves_from_banner(banner, target="nvd", limit=10)`** (new — splits a raw `Server:`/`X-Powered-By` header like `"Apache/2.4.7 (Ubuntu)"` into product/version first, dropping a non-version-looking second half rather than sending it to NVD as a bogus filter) | `cves: [{cve_id, product, version, description, cvss_score, cvss_severity, cvss_version, published_date, references[]}]`. Both now run a **product-disambiguation pass** (`_PRODUCT_ALIASES`: apache, nginx, iis, tomcat, openssh, vsftpd, proftpd, mysql, postgresql, lighttpd) that maps a bare vendor token to a more specific NVD keyword phrase *and* a CPE-product allowlist, then drops any CVE hit whose own CPE configuration data names a different product — found live: a bare `"Apache"` banner was returning an Apache **Groovy** RCE as a top match, same vendor, wrong product entirely. A CVE with no CPE data at all is kept (can't verify, don't assume wrong), only dropped when NVD *does* attach CPE data and none of it matches. Since 2026-07-31 a second gate, **version-range filtering** (`_cve_version_applicable()` / `_cpe_covers_version()` / `_version_tuple()`), answers the question the product gate can't: *does the DETECTED version actually fall in the vulnerable range?* NVD's `keywordSearch` is a plain AND-of-words match, so a CVE described "before 10.3" comes back for a host running exactly **10.3 — the fixed release** — because the string appears in the description. Found live in the 2026-07-29 verification pass: 7 false positives (5 "before 10.3" CVEs on an OpenSSH 10.3 host, 2 fixed-in-2.4.25 CVEs on Apache 2.4.25). The gate reads NVD's `configurations[].nodes[].cpeMatch[]` `versionStartIncluding/Excluding` + `versionEndIncluding/Excluding` bounds and drops a CVE **only when the detected version is definitively outside every vulnerable CPE range for the accepted product**. Same "cannot verify != does not match" principle throughout: no CPE data, a `*`/`-` version wildcard, a version too imprecise to place (a bare `"4"` against a `"before 4.1.22"` bound — `_version_cmp()` returns `None` for a proper-prefix comparison), or no detected version at all all **keep** the CVE. It over-reports before it under-reports, by construction. Both filters log their drop count (`filtered N CVE(s) whose vulnerable version range does not include 'X'`). |
...
- **sqlmap** — `_detect_injectable_candidates()` builds a **ranked, bounded** list (max 5) of
  gobuster/dirb paths with a script extension (`.php/.asp/.aspx/.jsp/.cgi`) at status 200/301/302,
  each with `?id=1` appended, via the shared `web_param_candidates()` ranker (promising names —
  `info`/`search`/`sqli`/… — before site roots like `index`/`home`). The dispatch probes them in
  order and **stops at the first sqlmap confirms injectable**. This replaced a first-`.php`-wins
  probe that stopped at `/index.php` (a non-injectable redirect) and never reached a genuinely
  injectable endpoint (e.g. `/info.php`) discovered in the same run. A confirmed injection then
  gets the read-only `--banner --current-db --dbs` enumeration (§3).
